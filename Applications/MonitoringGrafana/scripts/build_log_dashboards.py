...
def explorer():
    ...
    spec = {
        ...
        "panels": [
            ...
            logs("Pod diagnostics", POD + FILTER, "Pod-only view. Unpack restores Pod identity for inspection and displays the sanitized original message."),
...
def pipeline():
    ...
    spec = {
        ...
        "panels": [
            ...
            stat("Delivery drops in range", f'sum(increase(loki_write_dropped_entries_total{scoped}[$__range]))', "Entries abandoned after delivery failures, separate from deliberate privacy exclusions.", threshold="critical"),
            stat("Loki rejected entries", 'sum(increase(loki_discarded_samples_total{job="loki"}[$__range]))', "Shared-backend rejections, grouped by reason below. May be absent until first rejection.", threshold="critical"),
