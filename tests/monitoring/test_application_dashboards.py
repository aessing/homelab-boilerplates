...
class ApplicationDashboards(unittest.TestCase):
    def test_generated_layout_and_budget(self):
        ...
        for name, d in generated.items():
            with self.subTest(name=name):
                ...
                self.assertEqual([link['title'] for link in d['links']], ['Monitoring dashboards', 'Monitoring Applications'])
                self.assertEqual(d['links'][1]['tags'], ['monitoring-applications'])
    ...
    def test_missing_limits_and_homeassistant_semantics(self):
        ...
        targets = next(p for p in ds['authentik.json']['panels'] if p['title'] == 'Server, worker and outpost targets')
        self.assertIn('group_left(version)', targets['targets'][0]['expr'])
        self.assertIn('label_replace(kube_pod_container_info', targets['targets'][0]['expr'])
    ...
    def test_root_operations_center(self):
        ...
        self.assertEqual(d['time'], {'from': 'now-1h', 'to': 'now'})
        self.assertEqual(d['refresh'], '1m')
        self.assertEqual(d['links'][0]['title'], 'Monitoring dashboards')
        self.assertEqual([link for link in d['links'] if link['type'] == 'dashboards'], [d['links'][0]])
